package lascala.codejam.fourteen.one.a.a_charging_chaos
...
class ChargingChaosSpec extends Specification {
  ...
  def solve(L:Int, initial:List[String], expected:List[String]) = {
    val init = initial.map(java.lang.Long.parseLong(_,2))
    val expe = expected.map(java.lang.Long.parseLong(_, 2))
